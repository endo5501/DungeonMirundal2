...
- [x] 2.5 手動テスト: 全滅シナリオ → 町 → 再入場で入口スタートを確認
- [x] 2.6 手動テスト: 脱出の巻物 → 町 → 再入場で入口スタートを確認
- [x] 2.7 手動テスト: 緊急脱出の巻物 (戦闘中) → 町 → 再入場で入口スタートを確認
- [x] 2.8 手動テスト: ダンジョン中でセーブ → ロードで保存位置から開始することを確認
...
- [x] 3.4 手動テスト: ダンジョン内で入口付近を歩き、ミニマップに入口マーカーが表示されることを確認
...
- [x] 4.4 手動テスト: Godot エディタで実行し、入口セル上で 3D ビューに階段が立体的に描かれていることを確認

## 5. 最終検証

- [x] 5.1 すべてのテストスイートを実行して通過を確認する (1066 tests passing)
- [x] 5.2 入口が見えない・位置が残る問題が再現しないことをゲーム内で一連の流れ (通常探索 → 全滅 / 脱出巻物 / 自発帰還 → 再入場) で確認する
