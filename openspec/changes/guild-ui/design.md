...
```
GuildScreen (.tscn)  ← 親。Guildインスタンスを保持
├── GuildMenu.tscn         メニュー選択
├── CharacterCreation.tscn ウィザード5ステップ
├── PartyFormation.tscn    編成 + パーティ名変更
└── CharacterList.tscn     一覧・詳細・削除
```

GuildScreenが唯一のGuildインスタンスを保持し、表示する子シーンをインスタンス化して差し替える。各子画面はsignalで `back_to_menu` を通知する。

**理由:** 画面数が多くレイアウトが複雑なため、.tscnで管理する方がメンテナンス性が高い。差し替え方式はメモリ効率が良く、画面間の状態リセットが自然に行える。

**代替案:**
- 全画面をvisibility切り替え: メモリ消費が増え、状態管理が複雑化
- プログラム生成（既存パターン）: 画面数が多く可読性が低下
...
1画面内で5つのステップを順次表示する。各ステップはCharacterCreation.tscn内のサブControl切り替えで実現する。
...
```
src/guild_scene/
├── guild_screen.gd + .tscn       # 親画面。Guild保持、子シーン差し替え
├── guild_menu.gd + .tscn         # メニュー（4項目）
├── character_creation.gd + .tscn # 作成ウィザード（5ステップ内包）
├── party_formation.gd + .tscn    # パーティ編成 + 名前変更
└── character_list.gd + .tscn     # 一覧・詳細・削除
```
...
func _show_character_creation():
    var scene = character_creation_scene.instantiate()
    scene.setup(guild, data_loader.load_all_races(), data_loader.load_all_jobs())
    _switch_view(scene)
```
...
- **[.tscnの手動構築]** → GodotエディタなしでGDScript + .tscnを記述する場合、レイアウト調整が煩雑。対策: 最小限のレイアウトノード構成でプログラム的な配置を併用する
