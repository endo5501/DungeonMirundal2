...
- [ ] 4.1 main.gd に _load_game(slot_number) メソッドを追加し、SaveManager.load() 後に game_location に応じた画面を表示する
- [ ] 4.2 game_location="town" の場合の町画面復元を実装する
- [ ] 4.3 game_location="dungeon" の場合のダンジョン画面復元を実装する（seedからWizMap再生成 + ExploredMap/PlayerState適用）

## 5. セーブ画面UI

- [ ] 5.1 セーブ画面クラスを作成し、スロット一覧表示（番号、保存日時、パーティ名、最大レベル、現在地）を実装する
- [ ] 5.2 「新規保存」項目を一覧の先頭に表示し、選択時に次の連番で保存する
- [ ] 5.3 既存スロット選択時の上書き確認ダイアログを実装する
- [ ] 5.4 CursorMenuによるスロット選択操作を実装する
- [ ] 5.5 ESCキーで閉じる操作（back_requestedシグナル）を実装する

## 6. ロード画面UI

- [ ] 6.1 ロード画面クラスを作成し、スロット一覧表示を実装する（セーブ画面と同一の表示形式）
- [ ] 6.2 スロット選択時にload_requested(slot_number)シグナルを発行する
- [ ] 6.3 セーブファイルが存在しない場合の「セーブデータがありません」表示を実装する
- [ ] 6.4 ESCキーで閉じる操作（back_requestedシグナル）を実装する

## 7. ESCメニューとの統合

- [ ] 7.1 ESCメニューの「ゲームを保存」を有効化し、選択時にセーブ画面を表示する
- [ ] 7.2 ESCメニューの「ゲームをロード」を有効化し、選択時にロード画面を表示する
- [ ] 7.3 セーブ完了後にESCメニューに戻る処理を実装する
- [ ] 7.4 ロード選択後にESCメニューを閉じてゲーム画面を復元する処理を実装する
