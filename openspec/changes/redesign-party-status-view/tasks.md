...
- [ ] 7.3 手動確認: ダンジョン画面で ESC → パーティ → ステータスを開き、↑↓ でカーソルが動き右パネルが切り替わること、装備/呪文/EXP/ポートレイトが表示されること、メニュー表示中に WASD を押してもプレイヤーが動かないこと、ESC で正しくパーティメニューに戻ることを確認する。**※ ヘッドレス環境で実装したため未実施。ユーザによる動作確認が必要。**
- [ ] 7.4 ユーザの手動確認後、`/opsx:archive` でアーカイブする。
