...
- [x] 5.1 DungeonEntrance を実装する（ダンジョン一覧表示、カーソル選択、潜入/新規生成/破棄/戻るボタン、パーティ未編成時の潜入disabled）
- [x] 5.2 DungeonCreateDialog を実装する（サイズカテゴリ選択、ランダム名+編集、生成確認/キャンセル）

## 6. ダンジョン帰還

- [x] 6.1 DungeonScreen に STARTタイル検出ロジックを追加する（移動後にタイル判定）
- [x] 6.2 帰還確認ダイアログを実装する（はい/いいえ選択、ダイアログ中の入力ブロック、return_to_town シグナル）
