...
```
Image サイズ: 29 x 29 px（固定）
VIEW_RADIUS = 3, VIEW_SIZE = 7, CELL_PX = 3, WALL_PX = 1, STRIDE = 4

ビュー内セル(vx, vy) のピクセルマッピング:
  床   → 3x3 rect at (vx*4+1, vy*4+1)
  北壁 → 3px line at y=vy*4, x=[vx*4+1..vx*4+3]
  南壁 → 3px line at y=(vy+1)*4, x=[vx*4+1..vx*4+3]
  西壁 → 3px line at x=vx*4, y=[vy*4+1..vy*4+3]
  東壁 → 3px line at x=(vx+1)*4, y=[vy*4+1..vy*4+3]
  角   → ポストプロセスで隣接2辺以上が非背景なら塗る

プレイヤーは常に中心 pixel (14, 14) に描画。
マップ範囲外のセルは背景色のまま。
壁・ドアはラインで表現し、角ピクセルは描画しない（部屋内の柱を防止）。
```
...
- **ミニマップのスケーリング**: 29x29 ピクセルの Image を拡大表示するため、NEAREST フィルタが必須。拡大率によってはジャギーが目立つ → 表示サイズ 140px 程度に調整
