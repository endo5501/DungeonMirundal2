...
- [x] 8.1 `godot --editor` でプロジェクトを開き、ギルド > パーティ編成で待機リストと格子のカーソルが `"▶"` に変わっていることを目視確認
- [x] 8.2 街 > ダンジョン入口で起動時に `潜入する` にカーソルがあることを確認
- [x] 8.3 `潜入する` → Enter でリストに焦点が移ることを確認
- [x] 8.4 リスト焦点中にESCでボタン列に戻ることを確認
- [x] 8.5 `破棄` → Enter → リスト選択 → Enter → 確認ダイアログ → `はい`/`いいえ` の動作を確認
- [x] 8.6 空レジストリ状態で `新規生成` が初期フォーカスになることを確認
- [x] 8.7 空レジストリ時に `潜入する` / `破棄` が disabled 色で表示されることを確認

## 9. 仕上げ

- [x] 9.1 `godot --headless -s addons/gut/gut_cmdln.gd` で全テストがパスすることを確認
- [x] 9.2 変更内容をコミットする (spec 更新、実装、テストを論理単位でまとめる)
- [x] 9.3 PR作成 or main へのマージを検討する (`/opsx:archive` で change をアーカイブする)
