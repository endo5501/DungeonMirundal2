...
- [x] 9.1 戦闘を開始し、行動順にパネルが順次浮き上がる(lift)ことを確認
- [x] 9.2 ダメージを受けたパネルが横揺れ(shake)することを確認
- [x] 9.3 回復スキルを受けたパネルが緑にフラッシュすることを確認(戦闘外でも ESC 回復で同じく flash することを確認)
- [x] 9.4 HP 0 になるとパネルが薄くフェードし、暗転オーバーレイと組み合わさって "倒れた" 表現になることを確認
- [x] 9.5 蘇生でパネルが通常表示に戻ることを確認
- [x] 9.6 戦闘中にバフ/デバフ(STAT modifier)アイコンが出る・消えることを確認
- [x] 9.7 戦闘終了後、stat modifier アイコンが消えることを確認(detach されているか)
- [x] 9.8 ステータス付与時に persistent_status のアイコンが新たに出ることを確認
- [x] 9.9 連続被弾で shake が上書きされることを確認(視覚的にちらつかず、最後のダメージの shake で終わる)
