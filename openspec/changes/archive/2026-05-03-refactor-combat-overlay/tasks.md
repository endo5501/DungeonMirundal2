...
- [x] 2.1 `tests/combat/test_battle_resolver.gd` を作成、CLEARED outcome での経験値計算テスト
...
- [x] 3.1 既存 `tests/combat/test_combat_overlay.gd` のうち、戦闘終了時の経験値・ゴールド assert が BattleResolver 経由でも通ることを確認
- [x] 3.2 `src/dungeon_scene/combat_overlay.gd:_finalize_battle` を BattleResolver.resolve_rewards 呼び出しに置換
- [x] 3.3 `show_result(outcome, level_ups)` を `show_result(outcome, summary: BattleSummary)` に変更、result_panel への引数も更新
- [x] 3.4 `_compute_gold_drop`, `_collect_participant_characters`, `_collect_dead_monsters` を CombatOverlay から削除
- [x] 3.5 既存テスト全通過を確認しコミット
...
- [x] 5.1 既存テストが MenuController/InputRouter 経由でも通ることを確認
- [x] 5.2 `combat_overlay.gd:_unhandled_input` を `CombatInputRouter.route(event, _current_phase, _panels_dict())` 形に書き換え
- [x] 5.3 旧 `_handle_command_menu_key`, `_handle_target_select_key`, `_handle_item_select_key`, `_handle_result_key` を削除
...
- [x] 6.6 旧 `_open_item_selector`, `_on_item_selector_item_selected`, `_on_item_selector_cancelled`, `_valid_item_targets`, `_commit_item_command`, `_pending_item_instance` を削除
- [x] 6.7 `CombatItemSelector` クラスの扱いを判断:残すなら ItemUseFlow が内部で利用、削除するならテストも更新
- [x] 6.8 全テスト通過を確認しコミット
...
- [x] 8.6 `_on_result_confirmed` などで visible=false 時に `cancel_log_playback()` を呼ぶ
- [x] 8.7 テスト Green コミット

## 9. 動作確認

- [x] 9.1 `godot --headless -s addons/gut/gut_cmdln.gd` でフルテストスイート通過
- [x] 9.2 ゲーム起動 → ダンジョン → エンカウンタ → 全コマンド(攻撃/防御/アイテム/逃走)を試す
- [x] 9.3 アイテム使用フローが ItemUseFlow 経由で正しく動くこと
- [x] 9.4 死んだ敵への攻撃でリターゲット表示がログに出ること
- [x] 9.5 戦闘ログ再生中に高速で結果に進めるかをスタブで確認
- [x] 9.6 戦闘終了時の経験値・ゴールド・レベルアップ表示が正しいこと
