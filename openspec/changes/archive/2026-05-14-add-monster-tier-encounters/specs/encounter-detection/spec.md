...
**Reason**: `EncounterPattern` is replaced by the tier-weighted generation algorithm in `EncounterManager.generate`. Reusable groups are no longer needed because per-encounter species selection is driven by `tier_weights` directly.

**Migration**: Delete `src/dungeon/data/encounter_pattern.gd`, `src/dungeon/data/encounter_entry.gd`, and `src/dungeon/data/monster_group_spec.gd`. Replace `.tres` files under `data/encounter_tables/` with the new tier-based schema (see modified `EncounterTableData` requirement). Existing call sites that constructed `EncounterPattern` / `EncounterEntry` / `MonsterGroupSpec` (including tests) must be removed or rewritten to populate `tier_weights` and species/count ranges instead.

### Requirement: EncounterPattern expansion assigns each spawned monster to a row bucket

**Reason**: `EncounterPattern` is removed. The functional behavior (placing spawned monsters into FRONT/BACK buckets based on `MonsterData.default_row`) is preserved but now applies to `EncounterManager.generate` output directly.

**Migration**: The row bucket assignment is now implicit in the `EncounterManager.generate` algorithm and the truncation requirement. Each spawned `Monster` instance SHALL continue to be classified by `MonsterData.default_row`, and `MonsterCombatant.original_row` (set elsewhere in the combat layer) continues to use that value at combat construction time. No spec-level requirement is added here because the row-position semantics are now covered by the `combat-actor` capability and by the truncation requirement above.
