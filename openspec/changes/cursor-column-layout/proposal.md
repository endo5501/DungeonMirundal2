...
- **画面の呼び出しを行ベース API に移行する**: title / town / shop（top + buy/sell） / temple / dungeon-entrance / esc-menu / save / load / dungeon-screen（return dialog） / guild-menu / character-list / character-creation（Step2/3/4） / combat-command-menu / combat-target-selector を対象。
...
- **非対象**: combat_monster_panel / combat_result_panel、encounter_overlay、party_formation、dungeon_create_dialog。これらは独自の表示実装を持ち、レイアウトが特殊か CursorMenuRow への移行に追加設計が必要なため、別 change で扱う。
