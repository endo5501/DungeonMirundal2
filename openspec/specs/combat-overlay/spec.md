...
#### Scenario: CLEARED shows gained experience and level-up notifications
- **WHEN** the battle ends with `CLEARED` and any Character leveled up
- **THEN** the ResultPanel SHALL display the per-member gained experience and a line for each Character whose level increased, including the new level

#### Scenario: WIPED shows a defeat message
- **WHEN** the battle ends with `WIPED`
- **THEN** the ResultPanel SHALL display a defeat message and SHALL NOT display gained experience

#### Scenario: ESCAPED shows an escape message
- **WHEN** the battle ends with `ESCAPED`
- **THEN** the ResultPanel SHALL display an escape confirmation message and SHALL NOT display gained experience
