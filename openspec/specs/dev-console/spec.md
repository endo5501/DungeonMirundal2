## Purpose
開発者向けデバッグツール「Dev Console」のシェル構造とセーブデータ編集機能を定義する。本体ゲームから独立した Godot シーン (`tools/dev_console/main.tscn`) として動作し、ゲームバランス調整・装備品検証・呪文テストのために多様なキャラクター状態を素早く再現できるようにする。Phase 1 では Saves タブのみを対象とし、Phase 2 以降に Items / Spells / Monsters 等の `.tres` 編集タブを追加する想定。
