# monster-balance-curve Specification

## Purpose
モンスター戦闘4値(HP レンジ・攻撃・防御・敏捷)を「tier 成長曲線 × 種の役割係数 + 明示オーバーライド」で定式化するバランス定義(`data/balance/monster_curve.json`)と、そのロード・検証・計算を担う `MonsterCurve` モデルを規定する。
## Requirements
