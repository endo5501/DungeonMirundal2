## Purpose
Character の HP / MP / persistent_statuses を観測可能にし、フィールドが値変化したときだけ対応するシグナルを発火させることで、UI (PartyDisplay 等) が live な Character 参照に bind して自動再描画できるようにする土台を規定する。シリアライズ復元 (`from_dict`) 中はシグナル発火を抑制し、初期化中の偽の通知を防ぐ。
