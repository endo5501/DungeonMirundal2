...
static func hit_chance(attacker: CombatActor, target: CombatActor) -> float:
	if attacker == null or target == null:
		return 0.0
	# Re-clamp here as a defense-in-depth: subclasses may override the public
	# accessor and skip CombatActor's clamp.
...
# Strict less-than is the explicit rule (randf < chance → hit), so it gets a
# named seam for boundary testing.
...
static func calculate(attacker: CombatActor, target: CombatActor, rng: RandomNumberGenerator) -> DamageResult:
	...
	if not roll_hit(chance, rng.randf()):
		return DamageResult.new(false, 0)
